...
read_from_intensity<-function(targets_dir,DNAmType,UniqueID,
  ...
  id_pos_t<-id_pos[index_tf_detp];GSM_id_pos_t<-GSM_id_pos[index_tf_detp]
  ...
  merged_signals <- cbind(methyl, unmethyl);rm(methyl, unmethyl)
  temp_path <- tempdir()
  ...
  if(DNAmType=="450k"){
    ...
    GenomicMethylSet<- readGEORawFile(filename = paste0(temp_path,"/","merged_signals.csv"),
                                      pData=pData,

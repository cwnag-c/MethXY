...
methxy.vmp<-function(matrix,design,Sample_Group,cores=10,chunks=10){
  ...
  rm(Residuals)
  Mean_cases<-apply(resid_case, 1, function(x){mean(x,na.rm=T)})
  Variance_cases<-apply(resid_case, 1, function(x){var(x,na.rm=T)})
  Mean_controls<-apply(resid_control, 1, function(x){mean(x,na.rm=T)})
  Variance_controls<-apply(resid_control, 1,  function(x){var(x,na.rm=T)})
  ...
  res<-lapply(dat_list, function(l){
    res_tmp<-foreach(cg=rownames(l),
                     .combine=rbind,
                     .packages = c("s20x", "MethXY"),
                     .export = c("Sample_Group")
    ) %dopar%{VarCpG(l[cg,],Sample_Group,cg)}
    return(res_tmp)
  })
  stopCluster(cl)
  res<-bind_rows(res)

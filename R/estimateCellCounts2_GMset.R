...
#' @description This function is adapted from the `estimateCellCounts2` function
#'     in the FlowSorted.Blood.EPIC package.
#'     The original copyright belongs to the FlowSorted.Blood.EPIC authors
...
#' @param GenomicMethylSet GenomicMethylSet
#' @param referenceset referenceset
#' @param CustomCpGs CustomCpGs
#' @param returnAll returnAll
#' @param meanPlot meanPlot
#' @param verbose verbose
#' @param lessThanOne lessThanOne
#' @param cellcounts cellcounts
#'
                                     ...
                                     meanPlot = FALSE, verbose = TRUE, lessThanOne = FALSE, cellcounts = NULL)
{
  ...
  if (!is.null(referenceset)) {
    ...
    referenceRGset<-referenceset;rm(referenceset)
  ...
  newpd <- DataFrame(sampleNames = c(colnames(GenomicMethylSet), colnames(referenceRGset)),
                     studyIndex = rep(c("user", "reference"), times = c(ncol(GenomicMethylSet),
                                                                                    ncol(referenceRGset))))
  ...
  reference_sex %<>% select(predictedSex) %>% rename(Sex=predictedSex)%>% mutate(samplename=rownames(reference_sex))
  ...
  if (probeSelect != "IDOL") {
    ...
    if (requireNamespace("FlowSorted.Blood.EPIC", quietly = TRUE)) {
      ...
      stop("Please install 'FlowSorted.Blood.EPIC'")
    }
...
#' @param numProbes numProbes
#' @param compositeCellType compositeCellType

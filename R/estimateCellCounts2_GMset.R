...
estimateCellCounts2_GMset <- function(GenomicMethylSet, referenceset = NULL, CustomCpGs = NULL, returnAll = FALSE,
  ...
  if (rgPlatform != platform) {
    GenomicMethylSet <- convertArray(GenomicMethylSet,
  ...
  newpd <- DataFrame(
    sampleNames = c(colnames(GenomicMethylSet), colnames(referenceRGset)),
    studyIndex = rep(c("user", "reference"), times = c(
      ncol(GenomicMethylSet),
      ncol(referenceRGset)
    ))
  )
  ...
  restry <- try(
    {
      colData(GenomicMethylSet)[commoncolumn] <- mapply(
        FUN = as, colData(GenomicMethylSet)[commoncolumn],
